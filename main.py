...
async def on_message(message):
    ...
    if message.content == "$help":
        ...
        embed.set_footer(text="Bot created by @Proconsulates#9083 and @✨ TeaKe_smAL ✨#4826")
    ...
    if message.content == "$challenge":
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://www.reddit.com/r/dailyprogrammer/new.json?sort=hot') as r:
                res = await r.json()
                ...
                if len(valid_challenges) == 0:
                    embed.add_field(name="Something went wrong", value="Unable to fetch a challenge.")
                else:
